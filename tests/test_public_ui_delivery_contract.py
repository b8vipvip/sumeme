...
class PublicUIDeliveryContractTests(unittest.TestCase):
    def test_compose_uses_local_pinned_lobehub_image(self) -> None:
        ...
        self.assertIn("sumeme-lobehub:2.2.11-auth-control-1", lobe)
    ...
    def test_upstream_patch_is_exact_and_idempotent(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(patched.count(module.CONTROL_PANEL_COPY), 1)
    ...
    def test_ghs_preloads_patched_image_before_deployment(self) -> None:
        ...
        self.assertIn("LOBEHUB_IMAGE_TAG: sumeme-lobehub:2.2.11-auth-control-1", workflow)
        self.assertIn("repository: lobehub/lobehub", workflow)
        self.assertIn("cp -R web/control-panel", workflow)
        self.assertIn("python scripts/patch-lobehub-auth-assets.py", workflow)
        self.assertIn(module_line := "COPY --from=builder /app/public/sumeme-control", workflow)
        self.assertTrue(module_line)
    ...
    def test_health_check_rejects_missing_app_assets_or_control_panel(self) -> None:
        ...
        self.assertIn("/sumeme-control/index.html", health)
        self.assertIn("<title>SuMeMe 控制台</title>", health)
        self.assertIn("class AssetParser", smoke)
        self.assertIn("asset_failures", smoke)
        self.assertIn("return 1 if failures else 0", smoke)

    def test_control_panel_is_a_read_only_sanitized_dashboard(self) -> None:
        panel = CONTROL_PANEL.read_text(encoding="utf-8")

        self.assertIn("SuMeMe 控制台", panel)
        self.assertIn("/sumeme-health", panel)
        self.assertIn("ops-status/status/status.json", panel)
        self.assertIn("只读运维界面", panel)
        self.assertNotIn("GATEWAY_ADMIN_TOKEN", panel)
        self.assertNotIn("LETTA_SERVER_PASSWORD", panel)
        self.assertNotIn("RUSTFS_SECRET_KEY", panel)
